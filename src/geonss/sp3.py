...
def collapse_sparse_dimension(ds: xr.Dataset, dim: str) -> xr.Dataset:
    """
...
def correct_positions_with_antex(pos_ds: xr.Dataset, antex_ds: xr.Dataset) -> xr.Dataset:
    ...
    if 'offset' not in antex_ds.data_vars or 'valid_until' not in antex_ds.data_vars or 'sv' not in antex_ds.coords or 'frequency' not in antex_ds.coords or 'time' not in antex_ds.coords:
        raise ValueError("antex_ds Dataset is missing required variables/coordinates ('offset', 'valid_until', 'sv', 'frequency', 'time').")
    ...
    def _apply_phase_center_offset(ecef_positions: np.ndarray, neu_offsets: np.ndarray) -> np.ndarray:
        ...
        if ecef_positions.ndim != 2 or ecef_positions.shape[1] != 3:
            raise ValueError(f"Internal Error: ecef_positions must be an (N, 3) array, got shape {ecef_positions.shape}")
        ...
        if ecef_positions.shape[0] != neu_offsets.shape[0]:
            raise ValueError(f"Internal Error: Number of ECEF positions ({ecef_positions.shape[0]}) must match number of NEU offsets ({neu_offsets.shape[0]})")
    ...
    def _process_satellite_group(satellite_position_group: xr.Dataset):
        ...
        try:
            satellite_antex_data = antex_ds.sel(sv=satellite_id, frequency=default_frequency).dropna(dim='time', subset=['offset'], how='all')
        except KeyError:
        ...
        if satellite_antex_data.time.size == 0:
            raise ValueError(f"No valid ANTEX offset data found for SV={satellite_id}, Freq={default_frequency} after dropping NaNs.")


        # Pad-select ANTEX data corresponding to the position timestamps
        try:
            selected_antex_valid_until = satellite_antex_data.valid_until.sel(time=single_satellite_data.time, method='pad')
            selected_antex_offsets = satellite_antex_data.offset.sel(time=single_satellite_data.time, method='pad')
        except KeyError as e:
             raise ValueError(f"Error selecting ANTEX data for SV={satellite_id} at times {single_satellite_data.time.values}. Missing time points? Error: {e}")
        ...
        if np.any(selected_antex_valid_until.values < single_satellite_data.time.values):
            invalid_times = single_satellite_data.time[selected_antex_valid_until.values < single_satellite_data.time.values].values
    ...
    # Add attributes describing that the data has been corrected
    corrected_positions_ds['position'].attrs.update({
        'description': 'Satellite position corrected with ANTEX Phase Center Offset',
        'units': 'meters',
    })

    return corrected_positions_ds
...
def interpolate_orbit_positions(
    ...
    reduced_result = collapse_sparse_dimension(grouped_result, dim='time_bins')
    ...
    transposed_result = annotated_result.transpose('time', 'sv', ...)
